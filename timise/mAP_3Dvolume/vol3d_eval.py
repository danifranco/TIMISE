...
#             'counts': [T, R, K, A, M],
    ...
    def summarize(self):
        ...
        def _summarizeDets():
            ...
            stats[4] = _summarize(1, areaRng='small', iouThr=.75)#, maxDets=self.params.maxDets[2])
            stats[5] = _summarize(1, areaRng='medium', iouThr=.75)#, maxDets=self.params.maxDets[2])
            stats[6] = _summarize(1, areaRng='large', iouThr=.75)#, maxDets=self.params.maxDets[2])
            # no recall
            """
    ...
    def save_match_p(self, output_name=''):
        ...
        np.savetxt(self.output_name+'_match_p.txt', self.result_p, fmt=rowformat, header=header)

    def save_match_fn(self, output_name=''):
        header = '\tprediction \t|\t\tgt \t\n' + \
                    'ID\tSIZE\t| ID\tSIZE\tIoU \n' + '-'*40
        rowformat = '%d\t\t%4d\t\t%d\t%4d\t%.4f'
        np.savetxt(self.output_name+'_match_fn.txt', self.result_fn, fmt=rowformat, header=header)
