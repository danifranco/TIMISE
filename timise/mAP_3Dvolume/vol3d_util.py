...
def seg_iou3d(pred, gt, slices, areaRng=np.array([]), todo_id=None, chunk_size=100, crumb_size = -1, verbose=True):
    ...
    bbs = seg_bbox3d(pred, slices, uid = todo_id, chunk_size = chunk_size, verbose=verbose)[:,1:]
...
def seg_iou3d_sorted(pred, gt, score, slices, areaRng = [0,1e10], chunk_size = 250, crumb_size = -1, verbose=True):
    ...
    result_p, result_fn = seg_iou3d(pred, gt, slices, areaRng, pred_id[pred_id_sorted], chunk_size, crumb_size, verbose=verbose)
